...
- All resume content lives in one typed data module, `lib/data.ts` (or split by section under `lib/data/` if it grows large) — no CMS, no runtime GitHub API fetch, no build-time `fetch.js` hack. Static and simple.
...
3. **Featured Projects** — VoiceFlow, ReachInbox, AmeBot. Each: description, tech tags, GitHub link, live link where available.
4. **More Projects** — smaller secondary grid: Travel Together, Heritage Hues, Student Performance Prediction.
...
9. **Certifications** — Cisco CCNA (3 courses), Cisco Python Essentials 1 & 2, OCI Generative AI Professional, Google AI Essentials.
